...
static const unsigned int MAX_BLOCK_SIZE_GEN = MAX_BLOCK_SIZE/2;         // 1000KB  block soft limit
...
public:
    ...
    uint256 GetPoWHash(int height) const
    {
        ...
        if (fTestNet && height >= HF_MTP_HEIGHT_TESTNET){
            ...
            mtp_verifier(hashTarget, mtpMerkleRoot, nNonce, blockWithMTPProof, mtpProof, &thash, GetHashMTP());
